...
def go_to_goal(x_goal, y_goal):
    ...
    prev_error = distance
    prev_errora = desired_angle_goal
    while (True):
        ...
        P = distance
        I = I + distance
        D = distance - prev_error
        Pa = desired_angle_goal             
        Ia = Ia + desired_angle_goal
        Da = Da - prev_errora
        linear_speed = (P*kp + I*ki + D*kd)
        angular_speed = (Pa*kpa + Ia*kia + Da*kda)
        velocity_message.linear.x = linear_speed
        velocity_message.angular.z = angular_speed
        pub.publish(velocity_message)
        print(round(linear_speed,2))
        print(round(angular_speed,2))
        print ('x=', x, 'y=',y)
